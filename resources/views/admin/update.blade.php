    ...
    <div class="card w-full max-w-2xl bg-white shadow-xl rounded-xl p-8">
        ...
        <form action="/admin/edit/{{ $rooms->room_id }}" method="POST" class="space-y-6">
            ...
            <div class="form-control w-full">
                ...
                >
                    <option value="solo" {{ old('room_type', $rooms->room_type) === 'solo' ? 'selected' : '' }}>Solo</option>
                    <option value="family" {{ old('room_type', $rooms->room_type) === 'family' ? 'selected' : '' }}>Family</option>
                    <option value="deluxe_vip" {{ old('room_type', $rooms->room_type) === 'deluxe_vip' ? 'selected' : '' }}>Deluxe / VIP</option>
                </select>
                @error('room_type')
                    <span class="text-error mt-1 text-sm">{{ $message }}</span>
                @enderror
