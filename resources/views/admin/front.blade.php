            ...
            <div class="card w-80 bg-white shadow-lg hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 rounded-xl">
                <figure>
                    <img src="https://img.daisyui.com/images/stock/photo-1606107557195-0e29a4b5b4aa.webp"
                         alt="{{ $room->room_type }}" class="rounded-t-xl" />
                </figure>
                <div class="card-body">
                    <h2 class="card-title text-primary">{{ $room->room_type }}</h2>
                    <p>{{ $room->room_desc }}</p>
                    <p class="font-semibold mt-2">${{ $room->room_price }} / night</p>
                    <div class="card-actions justify-end gap-2 mt-4">
                        ...
                        <a href="{{ route('bookings.form', ['room_type' => $room->room_type]) }}"
                           class="btn btn-outline btn-primary btn-sm">Book Now</a>
                    </div>
                </div>
            </div>
        @endforeach
