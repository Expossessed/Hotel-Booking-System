...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // Try to cache Vite build assets by fetching manifest.json
      try {
        ...
        if (res.ok) {
          ...
          Object.values(manifest).forEach(entry => {
            if (entry && entry.file) urls.push('/build/' + entry.file);
            if (entry && entry.css) entry.css.forEach(c => urls.push('/build/' + c));
            if (entry && entry.assets) entry.assets.forEach(a => urls.push('/build/' + a));
          });

          // Also try to add the generated assets directory
          urls.forEach(u => {
            // ignore failures, individual assets might be absent in some envs
            cache.add(u).catch(() => {});
          });
        }
      } catch (err) {
...
// Fetch strategy: serve cached assets first, network for navigation with fallback.
  ...
  if (req.mode === 'navigate') {
    event.respondWith(
      fetch(req)
        ...
        .catch(() => caches.match(req).then(r => r || caches.match(FALLBACK_HTML)))
    );
    return;
  }
